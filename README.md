...
An n8n automation that scores every open HubSpot deal each morning, explains *why* a deal is at risk, maintains an operational action queue, and sends Slack alerts when a material risk change is detected.
...
HubSpot  →  Load prior state  →  Score & classify  →  Decide what changed
            ...
            Audit log & summary                  Action queue + Slack      Resolution
             (Excel / Google Sheets)              (only if changed)      (close the loop)
```
...
Deal risk and data quality are scored in **separate buckets**. Revenue risk therefore remains distinct from CRM hygiene, while the data-quality score still provides a useful measure of record completeness. A seven-day age guard prevents very new deals from being treated as materially risky simply because their CRM record has not yet been fully populated.
...
**6. Write the record, then notify.** The durable operational record is written before the Slack message fires, so a notification failure never loses the underlying assessment record.
...
|---|---|---|
| **Action queue** | Excel / Google Sheets | Operational work queue containing deal, owner, amount, score, level, trend, drivers, alert reason, recommended action and status |
| **Slack alert** | Slack | Linked deal name, owner, amount, score with level, why it fired, risk drivers and recommended action |
| **Assessment log** | Excel / Google Sheets | One assessment row per deal evaluation, including score breakdown and issue codes |
| **Alert log** | Excel / Google Sheets | One row per delivered alert, including alert reason, issue fingerprint, action source and execution ID |
| **Resolved log** | Excel / Google Sheets | Resolution history for deals that leave or change out of the tracked risk state |
| **Run summary** | Excel / Google Sheets | Run-level counts by risk level, high-value risk count, missing-close count, single-threading count and high/critical exposure |

### Workbook outputs

The sheet outputs are provided as the original Excel workbooks rather than screenshots because several tables are too wide to represent accurately in an image.

- [Risk Audit Log.xlsx](risk-audit-log.xlsx) — detailed assessment history for individual deal evaluations.
- [Ops.xlsx](Ops.xlsx) — operational outputs including the run summary, action queue, alert log and resolved log.

State used for change detection lives in n8n Data Tables, not in these workbooks. The workbooks are the durable audit and operational outputs; the Data Tables provide the workflow's working memory.

---

## Test-run evidence

The Excel outputs provide concrete evidence from the HubSpot test portal rather than only describing the intended architecture.
...
The detailed assessment log contains **268 assessment records covering 72 unique deals** across the available test history. Each assessment preserves the scoring and evidence behind the classification rather than only the final severity.
...
All 26 recorded alerts were sent using the `AI` action source and targeted `#revops-risk`.

This provides an audit trail of *why* an alert was delivered rather than only recording that a notification happened.
...
The queue also records whether a risk is `NEW` or `STABLE` and why the record was created or refreshed, including `NEW_RISK_ALERT` and `DUPLICATE_RISK_STATE`.

The queue therefore functions as an **operational work queue and history**, rather than simply being a second copy of the scoring output.
...
![Shared error handler](screenshots/04-error-handler-canvas.png)
...
├── README.md
├── RevOps Risk Audit Log.xlsx
├── Ops.xlsx
├── workflows/
│   ├── 01-deal-risk-system-draft.json      # first working version, 20 nodes
│   ├── 02-deal-risk-system-final.json      # current version, 32 nodes
...
All three workflow files import into n8n directly. Credentials and account-specific IDs are replaced with placeholders — `docs/setup.md` lists every value to substitute, the workbook outputs to inspect, and the Data Table schemas.

The repository includes the original working version because it is part of the project's development history; the **final workflow remains the primary implementation** documented by this README.
